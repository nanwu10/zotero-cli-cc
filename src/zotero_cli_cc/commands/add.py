...
def add_cmd(ctx: click.Context, doi: str | None, url: str | None) -> None:
    ...
    if not library_id or not api_key:
        click.echo(format_error(ErrorInfo(message="Write credentials not configured", context="add", hint="Run 'zot config init' to set up API credentials"), output_json=json_out))
        return
